...
const RecipeDetails: React.FC = () => {
  ...
  useEffect(() => {
    if (recipeId) {
      fetch(
        `/api/favorites/check/${encodeURIComponent(recipeId)}?itemType=recipe`,
        {
          credentials: "include"
        }
      )
    ...
    try {
      if (isFavorite) {
        ...
        const response = await fetch(
          `/api/favorites/${encodeURIComponent(recipeId)}?itemType=recipe`,
          {
            method: "DELETE",
            credentials: "include"
          }
  ...
  const handleAddRecipeToGroceryList = async (listId?: number) => {
    ...
    try {
      const response = await fetch(
        `/api/grocery-lists/${targetListId}/add-recipe`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json"
      ...
      toast.error(
        "Failed to add ingredients to grocery list. Please try again."
      );
    } finally {
    ...
    try {
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : "Failed to log recipe. Please try again."
      );
    } finally {
    ...
    >
      <div className="space-y-6">
          ...
          <div className="hidden sm:flex sm:flex-row sm:items-center sm:gap-3">
            <Dropdown>
              ...
              <DropdownMenu>
                ...
                {isOwnRecipe ? (
                  <DropdownItem onClick={handleEdit}>Edit Recipe</DropdownItem>
                ) : (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div
            className={`space-y-6 ${recipe.image_filename ? "lg:col-span-2" : "lg:col-span-3"}`}
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <section>
                  ...
                  <ul className="space-y-2">
                    {recipe.ingredients.map((ingredient, index) => {
                      ...
                      if (
                        displayIngredient.quantity !== null &&
                        displayIngredient.unit
                      ) {
                        displayIngredient = convertIngredient(
                          displayIngredient,
                          unitSystem
                        );
                      }
                      ...
                      if (displayIngredient.quantity !== null) {
                        const unitLabel = displayIngredient.unit
                          ? UNITS[displayIngredient.unit as keyof typeof UNITS]
                              ?.label || displayIngredient.unit
                          : "";
              ...
              {recipe.macros &&
                ...
                Object.keys(recipe.macros).length > 0 && (
                  <section>
                    ...
                    <div className="space-y-2">
                      {recipe.macros.calories !== undefined && (
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="font-medium text-zinc-900 dark:text-zinc-100">
                            {recipe.macros.calories.toFixed(0)} kcal
                          </span>
                        </div>
                      )}
                      {recipe.macros.protein !== undefined && (
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="font-medium text-zinc-900 dark:text-zinc-100">
                            {recipe.macros.protein.toFixed(1)} g
                          </span>
                        </div>
                      )}
                      {recipe.macros.carbohydrates !== undefined && (
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="font-medium text-zinc-900 dark:text-zinc-100">
                            {recipe.macros.carbohydrates.toFixed(1)} g
                          </span>
                        </div>
                      )}
                      ...
                      {recipe.macros.fiber !== undefined && (
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="font-medium text-zinc-900 dark:text-zinc-100">
                            {recipe.macros.fiber.toFixed(1)} g
                          </span>
                        </div>
                      )}
                      {recipe.macros.sugars !== undefined && (
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="font-medium text-zinc-900 dark:text-zinc-100">
                            {recipe.macros.sugars.toFixed(1)} g
                          </span>
                        </div>
                      )}
                      {recipe.macros.net_carbs !== undefined && (
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="font-medium text-zinc-900 dark:text-zinc-100">
                            {recipe.macros.net_carbs.toFixed(1)} g
                          </span>
                        </div>
                      )}
                      {recipe.macros.saturated_fat !== undefined && (
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="font-medium text-zinc-900 dark:text-zinc-100">
                            {recipe.macros.saturated_fat.toFixed(1)} g
                          </span>
                        </div>
                      )}
                      {recipe.macros.unsaturated_fat !== undefined && (
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="font-medium text-zinc-900 dark:text-zinc-100">
                            {recipe.macros.unsaturated_fat.toFixed(1)} g
                          </span>
                        </div>
                      )}
                      {recipe.macros.polyunsaturated_fat !== undefined && (
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="font-medium text-zinc-900 dark:text-zinc-100">
                            {recipe.macros.polyunsaturated_fat.toFixed(1)} g
                          </span>
                        </div>
                      )}
                      {recipe.macros.monounsaturated_fat !== undefined && (
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="font-medium text-zinc-900 dark:text-zinc-100">
                            {recipe.macros.monounsaturated_fat.toFixed(1)} g
                          </span>
                        </div>
                      )}
                      {recipe.macros.cholesterol !== undefined && (
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="font-medium text-zinc-900 dark:text-zinc-100">
                            {recipe.macros.cholesterol.toFixed(0)} mg
                          </span>
                        </div>
                      )}
                      {recipe.macros.sodium !== undefined && (
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="font-medium text-zinc-900 dark:text-zinc-100">
                            {recipe.macros.sodium.toFixed(0)} mg
                          </span>
                        </div>
                      )}
                      {recipe.macros.potassium !== undefined && (
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="font-medium text-zinc-900 dark:text-zinc-100">
                            {recipe.macros.potassium.toFixed(0)} mg
                          </span>
                        </div>
                      )}
                      {recipe.macros.protein_distribution_per_meal !==
                        undefined && (
                        <div className="flex justify-between text-sm">
                          <span className="text-zinc-600 dark:text-zinc-400">
                          ...
                          <span className="font-medium text-zinc-900 dark:text-zinc-100">
                            {recipe.macros.protein_distribution_per_meal.toFixed(
                              1
                            )}{" "}
                            g
                          </span>
      ...
      <Dialog
        ...
        onClose={setIsAddToListDialogOpen}
        size="xl"
      >
        <DialogTitle>Add Ingredients to Grocery List</DialogTitle>
        <DialogDescription>
          Add all ingredients from "{recipe.name}" to a grocery list. If an
          ingredient already exists, the quantities will be combined
          automatically.
        </DialogDescription>
        <DialogBody>
          <div className="space-y-6">
              ...
              <Field>
                ...
                <div className="flex flex-col gap-3 sm:flex-row">
                  ...
                  <Button
                    ...
                    disabled={
                      isCreatingList || isAddingToList || !newListName.trim()
                    }
      ...
      {/* Log Recipe Dialog */}
      <Dialog
        open={isLogRecipeDialogOpen}
        onClose={setIsLogRecipeDialogOpen}
        size="xl"
      >
        <DialogTitle>Log Recipe</DialogTitle>
        <DialogDescription>
          Log "{recipe.name}" to your food tracking. Select the number of
        ...
        <DialogActions>
          <Button
            plain
            onClick={() => setIsLogRecipeDialogOpen(false)}
            disabled={isLoggingRecipe}
          >
            Cancel
          </Button>
